...
image pidgeon, score, crosshair, background, end_splash; // all the layers we draw on
...
void fill_image(image* image, layer* layers, size_t numLayers, bool moving)
{
    ...
    int i;
    for (i = 0; i < numLayers; i++) { // expand max bounds of the image, which functions as a hitbox
...
{
    int k;
    for (k = 0; k < image->numLayers; k++) { // search all layers of each image
        ...
        short relx = j - image->layers[k].x0; // find relative indices
        short rely = i - image->layers[k].y0;
        short byte_index = (rely * bytes_per_row) + relx / 8;
        bool inbounds = relx >= 0 ? (relx < image_width ? (rely >= 0 ? rely < image_height : false) : false) : false; // check if layer contains this i,j coordinate
